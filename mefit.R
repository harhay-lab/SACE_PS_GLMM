## The "sace" function calculated the estimated SACE using
## the generalized linear mixed effects models
## Input: 
##      da: data set
##  y.name: name of the outcome variable
## tr.name: name of the treatment variable
## cluster.name: name of the cluster variable
## dist: distribution of the outcome variable (currently, 
##       only "binomial" distribution is supported)
...
# "cid" is the cluster id variable 

# In the control group, "cid" ranges from 1 to the number of clusters
...
# In the control group, "cid" also ranges from 1 to the number of clusters
...
io=ini(da1,da0,xnames) # assign starting values to the parameters

if(dist=="binomial") fobj=bfit(da1,da0,io) # model fitting
# currently, only binomial distributin is supported

#if(dist=="poisson") fobj=pfit(da1,da0,io)
#if(dist=="normal") fobj=nfit(da1,da0,io)

return(fobj) # the estimated SACE
}
